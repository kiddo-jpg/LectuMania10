...
@section('content')
    ...
    <main>
        <form class="max-w-md w-full" action="{{ route('usuarios.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <h1 class="text-2xl font-bold text-center mb-5 text-gray-800 dark:text-white">Ingresa tu usuario</h1>
            ...
            <div class="relative z-0 w-full mb-5 group">
                ...
                <div class="mt-2 text-xs text-gray-500 dark:text-gray-400 flex items-center space-x-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-blue-500 dark:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M12 2a10 10 0 100 20 10 10 0 000-20z" />
                    </svg>
                    <p>Debe tener al menos 8 caracteres, incluir una letra mayúscula y un carácter especial.</p>
                </div>
            </div>
            <div class="relative z-0 w-full mb-5 group">
                <input type="tel" name="telefono" id="telefono" pattern="[0-9]{10}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
            ...
            <div class="relative z-0 w-full mb-5 group">
                <label for="foto" class="block mt-2 text-sm text-gray-500 dark:text-gray-400">Subir Imagen</label>
                <br>
            ...
            <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 cursor-pointer">Ingresar</button>
            <a href="{{ route('login') }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 cursor-pointer">Iniciar Sesión</a>
        </form>
    </main>

    <!-- Mostrar errores en una ventana emergente -->
    @if ($errors->any())
        <script>
