...
<div class="container mx-auto px-4 py-6">
    ...
    @if(session('error'))
        <div class="mb-4 text-red-600 bg-red-100 border border-red-400 rounded px-4 py-2">
            {{ session('error') }}
        </div>
    @endif
        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                ...
                <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                    ...
                    <td class="px-4 py-3">{{ $pedido->usuario->nombre ?? 'N/A' }}</td>
                    <td class="px-4 py-3">{{ $pedido->libro->titulo ?? 'N/A' }}</td>
                    <td class="px-4 py-3">{{ $pedido->cantidad ?? 1 }}</td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3">
                        {{-- Ejemplo de botón de acción, puedes agregar más según permisos --}}
                        ...
                        <form action="{{ route('pedidos.destroy', $pedido->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-600 hover:underline ml-2" onclick="return confirm('¿Eliminar este pedido?')">Eliminar</button>
                        </form>
                        @endif
                    </td>
                </tr>
